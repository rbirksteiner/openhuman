...
fn build_registered_controllers() -> Vec<RegisteredController> {
    ...
    // ElevenLabs Conversational Agent — signed-URL, config, and session telemetry.
    controllers.extend(crate::openhuman::voice_agent::all_voice_agent_registered_controllers());
...
fn build_declared_controller_schemas() -> Vec<ControllerSchema> {
    ...
    // ElevenLabs Conversational Agent — signed-URL, config, and session telemetry.
    schemas.extend(crate::openhuman::voice_agent::all_voice_agent_controller_schemas());
    schemas
}
